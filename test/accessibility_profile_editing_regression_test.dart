...
void main() {
  ...
  void mockChannels() {
    ...
    messenger.setMockMethodCallHandler(
      const MethodChannel('com.example.mluvici_kalkulacka/tts_settings'),
      (MethodCall call) async => null,
    );
  }
  ...
  group('S1 aktivni edit save reopen restart', () {
    testWidgets('S1', (tester) async {
      ...
      state.updateEditingForTest(
        (AccessibilitySettings s) => s.copyWith(
          speechRate: 0.77,
          dialogFontScale: 2.2,
          speechVolume: 0.33,
        ),
      ...
      expect(
        state.editingDraftForTest.settings.speechRate,
        closeTo(0.77, 0.001),
      );
      expect(
        state.profilesForTest
            .firstWhere((p) => p.id == 'standard')
            .settings
            .speechRate,
        isNot(closeTo(0.77, 0.001)),
      );
    ...
    testWidgets('S2', (tester) async {
      ...
      expect(
        state.profilesForTest
            ...
            .settings
            .speechRate,
        closeTo(beforeRate, 0.001),
      );
      state.discardEditingForTest();
      await tester.pumpAndSettle();
      expect(state.editingDraftForTest, isNull);
      expect(
        state.dialogFontScaleNotifierForTest.value,
      ...
      final std =
          (decoded.firstWhere((e) => (e as Map)['id'] == 'standard')
                  as Map)['settings']
              as Map;
    ...
    testWidgets('S3', (tester) async {
      ...
      state.updateEditingForTest(
        (AccessibilitySettings s) =>
            s.copyWith(speechRate: 0.88, fontSizeMultiplier: 1.99),
      );
      await tester.pumpAndSettle();
  ...
  group('S4 neaktivni cancel bez runtime zmeny', () {
    testWidgets('S4', (tester) async {
      ...
      state.switchProfileForTest('standard');
      await tester.pumpAndSettle();
      final beforeNotifier =
          state.dialogFontScaleNotifierForTest.value as double;
      final beforeActiveRate = state.speechRateForTest as double;
      state.startEditingForTest('blind');
      await tester.pumpAndSettle();
      state.updateEditingForTest(
        (AccessibilitySettings s) =>
            s.copyWith(speechRate: 0.12, dialogFontScale: 2.8),
      );
      await tester.pumpAndSettle();
      // runtime unchanged
      expect(
        state.dialogFontScaleNotifierForTest.value,
        closeTo(beforeNotifier, 0.001),
      );
  ...
  group('S5 draft neaktivniho + zmena aktivniho zvenčí', () {
    testWidgets('S5', (tester) async {
      ...
      await tester.pumpAndSettle();
      state.updateEditingForTest(
        (AccessibilitySettings s) => s.copyWith(speechRate: 0.77),
      );
      await tester.pumpAndSettle();
      ...
      final lowBefore = state.profilesForTest
          .firstWhere((p) => p.id == 'lowvision')
          .settings
          .speechRate;
      final ok = await state.saveEditingForTest();
      expect(ok, isTrue);
  ...
  group('S6 reset behem otevreneho draftu', () {
    testWidgets(
      'S6 active reset updates draft and runtime, save persists defaults, cancel reverts to original',
      (tester) async {
        SharedPreferences.setMockInitialValues(<String, Object>{
          'modeQuestionAsked': true,
        });
        ...
        state.updateEditingForTest(
          (AccessibilitySettings s) =>
              s.copyWith(speechRate: 0.9, dialogFontScale: 2.2),
        );
        await tester.pumpAndSettle();
        expect(state.dialogFontScaleNotifierForTest.value, closeTo(2.2, 0.001));
        ...
        expect(
          state.editingDraftForTest.settings.dialogFontScale,
          closeTo(1.0, 0.001),
        );
        ...
        await tester.pumpAndSettle();
        state.updateEditingForTest(
          (AccessibilitySettings s) => s.copyWith(speechRate: 0.88),
        );
        await tester.pumpAndSettle();
        await state.resetProfileForTest('standard');
        await tester.pumpAndSettle();
  ...
  group('S7 race pred load', () {
    testWidgets('S7 dialog shows loading guard and startEditing validation', (
      tester,
    ) async {
      SharedPreferences.setMockInitialValues(<String, Object>{
        'modeQuestionAsked': true,
      ...
      expect(
        find.byWidgetPredicate(
          (w) => w is Text && (w.data == 'Upravit' || w.data == 'Edit'),
        ),
        findsOneWidget,
      );
  ...
  group('S9 Esc dismiss reverts active', () {
    testWidgets('S9', (tester) async {
      ...
      state.updateEditingForTest(
        (AccessibilitySettings s) =>
            s.copyWith(speechRate: 0.91, dialogFontScale: 2.0),
      );
      await tester.pumpAndSettle();
  ...
  group('S12 dialogFontScale immediate preview', () {
    testWidgets('S12', (tester) async {
      ...
      await tester.pumpAndSettle();
      state.updateEditingForTest(
        (AccessibilitySettings s) => s.copyWith(dialogFontScale: 1.8),
      );
      await tester.pumpAndSettle();
      expect(state.dialogFontScaleNotifierForTest.value, closeTo(1.8, 0.001));
      state.updateEditingForTest(
        (AccessibilitySettings s) => s.copyWith(dialogFontScale: 0.7),
      );
      await tester.pumpAndSettle();
  ...
  group('S13 semantics selected', () {
    testWidgets('S13', (tester) async {
      ...
      expect(
        find.bySemanticsLabel(RegExp(r'Standard.*vybrán')),
        findsOneWidget,
      );
      ...
      final standardSemanticsFinder = find.bySemanticsLabel(
        RegExp(r'Standard.*vybrán'),
      );
  ...
  group('S10 S11 focus', () {
    testWidgets('S10 TTS subdialog does not break draft', (tester) async {
      ...
      state.updateEditingForTest(
        (AccessibilitySettings s) => s.copyWith(speechVolume: 0.33),
      );
      await tester.pumpAndSettle();
      expect(state.editingDraftForTest, isNotNull);
      expect(
      ...
      expect(
        state.editingDraftForTest.settings.speechVolume,
        closeTo(0.33, 0.001),
      );
      state.discardEditingForTest();
      await tester.pumpAndSettle();
