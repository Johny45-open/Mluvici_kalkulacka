...
class _StatsSetsDialogState extends State<_StatsSetsDialog> {
  ...
  KeyEventResult _handleSortArchiveKey(FocusNode node, KeyEvent event) {
    ...
    if (event.logicalKey == LogicalKeyboardKey.space ||
      ...
      widget.parent._announce(
        _showArchived
            ? widget.parent._s('Archiv zapnut', 'Archive on')
            : widget.parent._s('Archiv vypnut', 'Archive off'),
      );
      return KeyEventResult.handled;
    }
    if (event.logicalKey == LogicalKeyboardKey.home) {
      setState(() => _sortBy = order.first);
      return KeyEventResult.handled;
    }
    if (event.logicalKey == LogicalKeyboardKey.end) {
      setState(() => _sortBy = order.last);
      return KeyEventResult.handled;
    }
    return KeyEventResult.ignored;
  }

  KeyEventResult _handleFolderKey(
    FocusNode node,
    KeyEvent event,
    ...
    if (event.logicalKey == LogicalKeyboardKey.arrowRight ||
      ...
      final label = next == null
          ? widget.parent._s('Vše', 'All')
          : next == '__none'
          ? widget.parent._s('Bez složky', 'No folder')
          : folders[ids.indexOf(next) - 2].name;
    ...
    if (event.logicalKey == LogicalKeyboardKey.home) {
      ...
      widget.parent._announce(widget.parent._s('Vše', 'All'));
      return KeyEventResult.handled;
    }
    if (event.logicalKey == LogicalKeyboardKey.end) {
      setState(() => _selectedFolderId = ids.last);
      return KeyEventResult.handled;
    }
    return KeyEventResult.ignored;
  }

  KeyEventResult _handleSetsKey(
    FocusNode node,
    KeyEvent event,
    List<StatisticsSet> filtered,
  ) {
    if (event is! KeyDownEvent) return KeyEventResult.ignored;
    ...
    if (char != null &&
      ...
      int found = -1;
      for (int i = 0; i < len; i++) {
        final idx = (start + i) % len;
        if (filtered[idx].name.toLowerCase().startsWith(_typeAhead)) {
          found = idx;
          break;
        }
      }
      if (found == -1)
        for (int i = 0; i < len; i++)
          if (filtered[i].name.toLowerCase().startsWith(_typeAhead)) {
            found = i;
            break;
          }
      if (found != -1) {
        setState(() => _setsFocusIdx = found);
        widget.parent._announce('\, \ z ');
        return KeyEventResult.handled;
      }
      return KeyEventResult.handled;
    }
    if (event.logicalKey == LogicalKeyboardKey.arrowDown) {
      if (_setsFocusIdx < len - 1) {
        setState(() => _setsFocusIdx++);
        widget.parent._announce('\, \ z ');
      }
      return KeyEventResult.handled;
    }
    ...
    if (event.logicalKey == LogicalKeyboardKey.home) {
      setState(() => _setsFocusIdx = 0);
      widget.parent._announce('\, 1 z ');
      return KeyEventResult.handled;
    }
    if (event.logicalKey == LogicalKeyboardKey.end) {
      setState(() => _setsFocusIdx = len - 1);
      widget.parent._announce('\, \ z ');
      return KeyEventResult.handled;
    }
    if (event.logicalKey == LogicalKeyboardKey.enter ||
        event.logicalKey == LogicalKeyboardKey.space) {
      final set = filtered[_setsFocusIdx];
      final realIndex = widget.parent._statsSets.indexOf(set);
      final countForm = widget.parent._getStatsCountForm(set.records.length);
      widget.parent.setState(
        () => widget.parent._currentStatsSetIndex = realIndex,
      );
      widget.parent._saveStatsData();
      final msg = widget.parent._l10n.statsSetSelectedAnnouncement(
        set.name,
        set.records.length,
        countForm,
      );
  ...
  void _showCreateChoice() {
    ...
    parent.showAppDialog<void>(
      ...
      builder: (ctx) => SimpleDialog(
        title: Semantics(
          header: true,
          child: Text(parent._s('Vytvořit sadu', 'Create set')),
        ),
        children: [
          SimpleDialogOption(
            onPressed: () {
            ...
            child: Semantics(
              label: parent._s(
                'Rychlé vytvoření – název a pole',
                'Quick create – name and fields',
              ),
              child: Text(parent._s('Rychlé vytvoření', 'Quick create')),
            ),
          ),
          SimpleDialogOption(
            onPressed: () {
            ...
            child: Semantics(
              ...
              child: Text(parent._s('Průvodce', 'Wizard')),
            ),
          ),
          SimpleDialogOption(
            onPressed: () {
            ...
            child: Semantics(
              label: parent._s('Hlasové vytvoření', 'Voice create'),
              child: Text(parent._s('Hlasem', 'By voice')),
            ),
          ),
        ],
      ),
    ...
    return Semantics(
      ...
      child: DropdownButton<String>(
        ...
        items: [
          DropdownMenuItem(
            value: 'lastUsed',
            child: Text(parent._s('Poslední použití', 'Last used')),
          ),
          DropdownMenuItem(
            value: 'name',
            child: Text(parent._s('Název', 'Name')),
          ),
          DropdownMenuItem(
            value: 'count',
            child: Text(parent._s('Počet', 'Count')),
          ),
        ],
    ...
    return SizedBox(
      ...
      child: ListView(
        ...
        children: [
          ...
          ...parent._statsFolders.map(
            (f) => Padding(
              padding: const EdgeInsets.only(right: 6),
              child: ChoiceChip(
                avatar: CircleAvatar(
                  backgroundColor: parent._statsColorFor(f.colorIndex),
                  radius: 10,
                  child: Icon(
                    parent._statsIconFor(f.iconName),
                    size: 10,
                    color: Colors.white,
                  ),
                ),
  ...
  PopupMenuButton<String> _buildSetPopup(
    StatisticsSet set,
    int realIndex,
    bool isCurrent,
  ) {
    ...
    return PopupMenuButton<String>(
      ...
      onSelected: (v) async {
        switch (v) {
          ...
          case 'move':
            ...
            break;
          case 'copy':
            parent._showCopyStatsSetToFolderDialog(
              context,
              realIndex,
              _refresh,
            );
          ...
          case 'archive':
            ...
            break;
          case 'delete':
            parent._showDeleteStatsSetConfirmation(
              context,
              realIndex,
              _refresh,
            );
      ...
      itemBuilder: (_) => [
        PopupMenuItem(
          ...
          child: Text(
            isCurrent
                ? parent._s('✓ Aktivní', '✓ Active')
                : parent._s('Vybrat', 'Select'),
          ),
        ),
        PopupMenuItem(
          value: 'rename',
          child: Text(parent._s('Přejmenovat', 'Rename')),
        ),
        PopupMenuItem(
          value: 'edit',
          child: Text(parent._s('Upravit pole', 'Edit fields')),
        ),
        PopupMenuItem(
          value: 'color',
          child: Text(parent._s('Barva/ikona', 'Color/icon')),
        ),
        PopupMenuItem(
          value: 'move',
          child: Text(parent._s('Přesunout do složky', 'Move to folder')),
        ),
        PopupMenuItem(
          value: 'copy',
          child: Text(parent._s('Kopírovat do složky', 'Copy to folder')),
        ),
        PopupMenuItem(
          value: 'duplicate',
          child: Text(parent._s('Duplikovat', 'Duplicate')),
        ),
        PopupMenuItem(
          value: 'pin',
          child: Text(
            set.pinned
                ? parent._s('Odepnout', 'Unpin')
                : parent._s('Připnout', 'Pin'),
          ),
        ),
        PopupMenuItem(
          value: 'archive',
          child: Text(
            set.archived
                ? parent._s('Obnovit', 'Restore')
                : parent._s('Archivovat', 'Archive'),
          ),
        ),
        PopupMenuItem(
          value: 'delete',
          child: Text(
            parent._s('Smazat', 'Delete'),
  ...
  Widget _buildSetTileWide(StatisticsSet set, int realIndex, bool isCurrent) {
    ...
    return Card(
      ...
      child: ListTile(
        ...
        leading: Semantics(
          label: parent._s(
            'Sada ${set.name}, ${set.records.length} hodnot, složka ${parent._statsFolderName(set.folderId)}',
            'Set ${set.name}, ${set.records.length} values, folder ${parent._statsFolderName(set.folderId)}',
          ),
          child: CircleAvatar(
            backgroundColor: parent._statsColorFor(set.colorIndex),
            child: Icon(
              parent._statsIconFor(set.iconName),
              color: Colors.white,
              size: 18,
            ),
          ),
        ...
        title: Row(
          children: [
            ...
            if (set.pinned)
              Semantics(
                label: parent._s('Připnuto', 'Pinned'),
                child: const Padding(
            ...
            if (set.archived)
              Semantics(
                label: parent._s('Archivováno', 'Archived'),
                child: const Padding(
                  padding: EdgeInsets.only(left: 4),
          ...
          children: [
            Semantics(
              ...
              child: IconButton(
                ...
                onPressed: () => parent._showEditStatsSetDialog(
                  context,
                  realIndex,
                  _refresh,
                ),
  ...
  Widget build(BuildContext context) {
    ...
    List<StatisticsSet> filtered = parent._statsSets.where((s) {
      ...
      if (_selectedFolderId != null &&
          _selectedFolderId != '__none' &&
          s.folderId != _selectedFolderId)
        return false;
    ...
    final contentPadding = narrowScreen
                  ...
                  child: Padding(
                    ...
                    child: TextField(
                      decoration: InputDecoration(
                        labelText: parent._s('Hledat', 'Search'),
                  ...
                  children: [
                    Semantics(
                      ...
                      child: FilledButton.icon(
                        onPressed: () =>
                            parent._showCreateStatsSetDialog(context),
                    ...
                    Semantics(
                      label: parent._s(
                        'Vytvořit sadu – možnosti',
                        'Create set – options',
                      ),
                      button: true,
                      child: OutlinedButton.icon(
                        onPressed: _showCreateChoice,
                        icon: const Icon(Icons.more_horiz, size: 16),
                        label: Text(parent._s('Více', 'More')),
                      ),
                    ),
                    Semantics(
                      ...
                      child: OutlinedButton.icon(
                        onPressed: () =>
                            parent._showManageFoldersDialog(context, _refresh),
                        icon: const Icon(Icons.folder_copy, size: 16),
                        label: Text(parent._s('Složky', 'Folders')),
                      ),
                    ),
                  ],
                );
                Widget buildItem(BuildContext ctx, int index) {
                  ...
                  return LayoutBuilder(
                    builder: (itemCtx, itemConstraints) {
                      final itemNarrow =
                          itemConstraints.maxWidth < 420 || narrowContent;
                      if (itemNarrow) {
                  ...
                  if (filtered.isEmpty) {
                    return Center(
                      child: Semantics(
                        ...
                        child: Text(
                          parent._s(
                            'Žádné sady neodpovídají filtru',
