  ...
  Widget build(BuildContext context) {
    ...
    return StatefulBuilder(
      builder: (dialogContext, setDialogState) {
        ...
        final fieldUnit =
            p._statsSets.isNotEmpty &&
                p._selectedFieldIndex <
                    p._statsSets[p._currentStatsSetIndex].fieldUnits.length
            ? p._statsSets[p._currentStatsSetIndex].fieldUnits[p
                  ._selectedFieldIndex]
            : null;
        ...
        final allValues = sortedValues
            .map((v) {
              ...
              final unitStr = fieldUnit != null
                  ? ' ${p._getUnitSpeech(fieldUnit, value: v)}'
                  : '';
        ...
        final cvText = snapshot.cv == null
            ? 'Err'
            : '${p._formatNumberSmart(snapshot.cv!)} %';
        final wmeanText = snapshot.wmean == null
            ? null
            : p._formatNumberSmart(snapshot.wmean!);
        ...
        MapEntry<String, String>? entryForItem(StatsComputedItem it) {
          switch (it) {
            case StatsComputedItem.mean:
              return MapEntry(
                l10n.statsMean,
                p._formatNumberSmart(snapshot.mean),
              );
            case StatsComputedItem.sum:
              return MapEntry(
                l10n.statsSum,
                p._formatNumberSmart(snapshot.sum),
              );
            case StatsComputedItem.variance:
              return MapEntry(
                l10n.statsVariance,
                p._formatNumberSmart(snapshot.variance),
              );
            case StatsComputedItem.sd:
              return MapEntry(
                l10n.statsStdDev,
                p._formatNumberSmart(snapshot.sd),
              );
            case StatsComputedItem.median:
              return MapEntry(
                l10n.statsMedian,
                p._formatNumberSmart(snapshot.median),
              );
            case StatsComputedItem.min:
              return MapEntry(
                l10n.statsMin,
                p._formatNumberSmart(snapshot.min),
              );
            case StatsComputedItem.max:
              return MapEntry(
                l10n.statsMax,
                p._formatNumberSmart(snapshot.max),
              );
        ...
        void toggleAutoRead(bool? v) {
          ...
          final statusMsg = newVal
              ? p._s(
                  'Automatické čtení souhrnu zapnuto',
                  'Auto-read summary enabled',
                )
              : p._s(
                  'Automatické čtení souhrnu vypnuto',
                  'Auto-read summary disabled',
                );
          final hint = newVal
              ? p._s(
                  'Po otevření se rovnou přečte celý souhrn v nastaveném pořadí plus nápověda pro Tab.',
                  'On open the full summary in the configured order plus Tab hint will be read.',
                )
              : p._s(
                  'Po otevření se řekne jen hlavička s nápovědou pro Tab.',
...
/// Jeden řádek vypočtené statistiky jako skutečný fokusovatelný prvek.
///
/// Každá instance vlastní svůj [FocusNode] (vytvořený v [initState],
/// uvolněný v [dispose] – nikdy v `build`), takže Tab / Shift+Tab prochází
/// řádky standardním [FocusTraversalGroup] mechanismem bez ručního
/// zachytávání klávesy Tab. Vnitřní texty nejsou samostatné Tab stopy,
/// čtečka dostane jednu logickou položku („Název: hodnota, řádek i z N").
class _ComputedStatRow extends StatefulWidget {
  final String name;
  final String value;
  ...
  @override
  State<_ComputedStatRow> createState() => _ComputedStatRowState();
}

class _ComputedStatRowState extends State<_ComputedStatRow> {
  late final FocusNode _node = FocusNode(debugLabel: 'statRow${widget.index}');
