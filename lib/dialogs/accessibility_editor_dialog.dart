...
class _AccessibilityProfileEditorDialogState extends State<_AccessibilityProfileEditorDialog> {
  ...
  void _adjustDialogFontScale(double delta) {
    ...
    setState((){});
    parent.speak(parent._s('Velikost písma dialogů ${(nv*100).toInt()} procent','Dialog font size ${(nv*100).toInt()} percent'), force: true);
  }
  void _adjustKeyboardFontScale(double delta) {
    ...
    setState((){});
    parent.speak(parent._s('Velikost písma tlačítek ${(nv*100).toInt()} procent','Keyboard button font size ${(nv*100).toInt()} percent'), force: true);
  }
  void _adjustOverlineHeight(double delta) {
    ...
    setState((){});
    parent.speak(parent._s('Výška periodické čáry ${(nv*100).toInt()} procent','Repeating bar height ${(nv*100).toInt()} percent'), force: true);
  }
  void _adjustOverlineThickness(double delta) {
    ...
    setState((){});
    parent.speak(parent._s('Tloušťka periodické čáry ${(nv*100).toInt()} procent','Repeating bar thickness ${(nv*100).toInt()} percent'), force: true);
  }
  void _resetOverlineStyle() {
    _onUpdate((s)=> s.copyWith(overlineHeight: 1.0, overlineThickness: 1.0));
    setState((){});
    parent.speak(parent._s('Vzhled periodické čáry obnoven','Repeating bar appearance reset'), force: true);
  }
  ...
  List<String> _collectChanges() {
    ...
    if (orig == null) return [parent._s('Změny v profilu $editingName','Changes in profile $editingName')];
    final out = <String>[];
    void add(String cs, String en, bool changed) { if (changed) out.add(parent._s(cs,en)); }
    add('Typ displeje: ${draft.useSixteenSegment ? '16-segment' : '7-segment'}', 'Display type: ${draft.useSixteenSegment ? '16-segment' : '7-segment'}', draft.useSixteenSegment != orig.useSixteenSegment);
    add('Periodický zápis: ${draft.usePeriodicNotation ? 'Zapnuto' : 'Vypnuto'}', 'Repeating notation: ${draft.usePeriodicNotation ? 'On' : 'Off'}', draft.usePeriodicNotation != orig.usePeriodicNotation);
    add('Hlasový výstup: ${draft.ttsEnabled ? 'Zapnuto' : 'Vypnuto'}', 'Voice output: ${draft.ttsEnabled ? 'On' : 'Off'}', draft.ttsEnabled != orig.ttsEnabled);
    add('Oznamování příkladu: ${draft.announceExpression ? 'Zapnuto' : 'Vypnuto'}', 'Announce expression: ${draft.announceExpression ? 'On' : 'Off'}', draft.announceExpression != orig.announceExpression);
    add('Auto-čtení souhrnu: ${draft.autoReadStatsSummary ? 'Zapnuto' : 'Vypnuto'}', 'Auto-read summary: ${draft.autoReadStatsSummary ? 'On' : 'Off'}', draft.autoReadStatsSummary != orig.autoReadStatsSummary);
    add('Nápověda Tab: ${draft.showStatsNavigationHint ? 'Zapnuto' : 'Vypnuto'}', 'Tab hint: ${draft.showStatsNavigationHint ? 'On' : 'Off'}', draft.showStatsNavigationHint != orig.showStatsNavigationHint);
    if (draft.screenReaderMode != orig.screenReaderMode) out.add(parent._s('Režim čtečky: ${draft.screenReaderMode.name}','Screen reader: ${draft.screenReaderMode.name}'));
    if (draft.dialogSize != orig.dialogSize) out.add(parent._s('Velikost dialogů: ${draft.dialogSize.name}','Dialog size: ${draft.dialogSize.name}'));
    if ((draft.fontSizeMultiplier - orig.fontSizeMultiplier).abs() > 0.001) out.add(parent._s('Písmo tlačítek: ${(draft.fontSizeMultiplier*100).toInt()}%','Button font: ${(draft.fontSizeMultiplier*100).toInt()}%'));
    if ((draft.dialogFontScale - orig.dialogFontScale).abs() > 0.001) out.add(parent._s('Písmo dialogů: ${(draft.dialogFontScale*100).toInt()}%','Dialog font: ${(draft.dialogFontScale*100).toInt()}%'));
    if ((draft.dotMatrixZoom - orig.dotMatrixZoom).abs() > 0.001) out.add(parent._s('Zoom horního displeje: ${(draft.dotMatrixZoom*100).toInt()}%','Upper zoom: ${(draft.dotMatrixZoom*100).toInt()}%'));
    if ((draft.resultZoom - orig.resultZoom).abs() > 0.001) out.add(parent._s('Zoom výsledku: ${(draft.resultZoom*100).toInt()}%','Result zoom: ${(draft.resultZoom*100).toInt()}%'));
    if ((draft.speechRate - orig.speechRate).abs() > 0.001) out.add(parent._s('Rychlost řeči: ${(draft.speechRate*100).toInt()}%','Speech rate: ${(draft.speechRate*100).toInt()}%'));
    if ((draft.speechVolume - orig.speechVolume).abs() > 0.001) out.add(parent._s('Hlasitost: ${(draft.speechVolume*100).toInt()}%','Volume: ${(draft.speechVolume*100).toInt()}%'));
    if ((draft.overlineHeight - orig.overlineHeight).abs() > 0.001 || (draft.overlineThickness - orig.overlineThickness).abs() > 0.001) out.add(parent._s('Periodická čára: výška ${(draft.overlineHeight*100).toInt()}% tloušťka ${(draft.overlineThickness*100).toInt()}%','Repeating bar: height ${(draft.overlineHeight*100).toInt()}% thickness ${(draft.overlineThickness*100).toInt()}%'));
    if (draft.thousandGroupGap != orig.thousandGroupGap) out.add(parent._s('Mezera skupin: ${draft.thousandGroupGap.name}','Group gap: ${draft.thousandGroupGap.name}'));
    if (draft.alignInputLeft != orig.alignInputLeft) out.add(parent._s('Zarovnání vstupu: ${draft.alignInputLeft ? 'vlevo' : 'střed'}','Input align: ${draft.alignInputLeft ? 'left' : 'center'}'));
    if (draft.accessibilityType != orig.accessibilityType) out.add(parent._s('Typ přístupnosti: ${draft.accessibilityType.name}','Accessibility: ${draft.accessibilityType.name}'));
    if (draft.ttsEngine != orig.ttsEngine) out.add(parent._s('Engine: ${draft.ttsEngine ?? 'Výchozí'}','Engine: ${draft.ttsEngine ?? 'Default'}'));
    if (draft.ttsVoiceName != orig.ttsVoiceName) out.add(parent._s('Hlas: ${draft.ttsVoiceName ?? 'Výchozí'}','Voice: ${draft.ttsVoiceName ?? 'Default'}'));
    if (draft.inverseFormatPreference != orig.inverseFormatPreference) out.add(parent._s('Formát úhlů: ${draft.inverseFormatPreference==0 ? 'DMS' : 'Desetinné'}','Angle format: ${draft.inverseFormatPreference==0 ? 'DMS' : 'Decimal'}'));
  ...
  Future<void> _confirmAndSave() async {
    ...
    final isNoChange = changes.length==1 && (changes.first.contains('Žádné změny') || changes.first.contains('No changes'));
    final summary = changes.join('; ');
    final spoken = parent._s('Potvrdit uložení profilu $editingName. Změny: $summary','Confirm saving profile $editingName. Changes: $summary');
    final confirmed = await parent.showAppDialog<bool>(
      ...
      builder: (ctx) {
        ...
        return AlertDialog(
          ...
          title: Semantics(header: true, child: Text(parent._s('Potvrdit uložení','Confirm save'))),
          content: SingleChildScrollView(
            child: Semantics(
              ...
              child: Column(
                ...
                children: [
                  Text(parent._s('Profil: $editingName','Profile: $editingName'), style: const TextStyle(fontWeight: FontWeight.bold)),
                  const SizedBox(height: 8),
                  if (isNoChange)
                    Text(changes.first, style: const TextStyle(fontStyle: FontStyle.italic))
                  else
                    ...changes.map((c)=> Padding(padding: const EdgeInsets.symmetric(vertical:2), child: Row(crossAxisAlignment: CrossAxisAlignment.start, children:[const Text('• '), Expanded(child: Text(c))]))),
                  const SizedBox(height: 12),
                  Text(parent._s('Uložit změny?','Save changes?'), style: const TextStyle(fontWeight: FontWeight.w500)),
                ],
              ),
            ),
          ),
          actions: [
            TextButton(onPressed: ()=> Navigator.pop(ctx,false), child: Text(parent._l10n.cancel)),
            FilledButton(autofocus: true, onPressed: ()=> Navigator.pop(ctx,true), child: Text(parent._s('Uložit','Save'))),
          ],
        );
      },
    );
    ...
    parent.speak(parent._s('Profil $editingName uložen','Profile $editingName saved'), force: true);
  }
  @override
  Widget build(BuildContext context) {
    ...
    return PopScope(
      ...
      content: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            const Divider(),
            Semantics(label: parent._s('Přepnutí typu displeje','Switch display type'), child: ElevatedButton(onPressed: () { _onUpdate((v)=> v.copyWith(useSixteenSegment: !v.useSixteenSegment)); final nv = editingSettings.useSixteenSegment; parent.speak(nv ? parent._l10n.segment16On : parent._l10n.segment7On, force: true); setState((){}); }, child: Text(parent._l10n.displayType(s.useSixteenSegment ? parent._s('16-segmentový','16-segment') : parent._s('7-segmentový','7-segment'))))),
            const Divider(),
            Semantics(label: parent._s('Přepnutí periodického zápisu výsledků','Switch repeating decimal notation for results'), child: ElevatedButton(onPressed: () { _onUpdate((v)=> v.copyWith(usePeriodicNotation: !v.usePeriodicNotation)); final nv = editingSettings.usePeriodicNotation; parent.speak(parent._s('Periodický zápis: ${nv ? 'Zapnuto' : 'Vypnuto'}','Repeating notation: ${nv ? 'On' : 'Off'}'), force: true); setState((){}); }, child: Text(parent._s('Periodický zápis výsledků','Repeating decimal notation for results') + ': ' + (s.usePeriodicNotation ? parent._s('Zapnuto','On') : parent._s('Vypnuto','Off'))))),
            const Divider(),
            Semantics(label: parent._s('Přepnutí hlasového výstupu','Switch voice output'), child: ElevatedButton(onPressed: () { _onUpdate((v)=> v.copyWith(ttsEnabled: !v.ttsEnabled)); final nv = editingSettings.ttsEnabled; parent.speak(nv ? parent._l10n.voiceOn : parent._l10n.voiceOff, force: true); setState((){}); }, child: Text(parent._l10n.voiceOutput(s.ttsEnabled ? parent._s('Zapnuto','On') : parent._s('Vypnuto','Off'))))),
            const Divider(),
            Semantics(label: parent._s('Přepnutí oznamování příkladu před výpočtem','Switch announcing expression before calculation'), child: ElevatedButton(onPressed: () { _onUpdate((v)=> v.copyWith(announceExpression: !v.announceExpression)); final nv = editingSettings.announceExpression; parent.speak(parent._l10n.announceExpressionState(nv ? parent._s('Zapnuto','On') : parent._s('Vypnuto','Off')), force: true); setState((){}); }, child: Text(parent._l10n.announceExpressionState(s.announceExpression ? parent._s('Zapnuto','On') : parent._s('Vypnuto','Off'))))),
            const Divider(),
            Semantics(label: parent._s('Přepnutí automatického čtení statistického souhrnu při otevření','Toggle auto-read of statistics summary on open'), child: ElevatedButton(onPressed: () { _onUpdate((v)=> v.copyWith(autoReadStatsSummary: !v.autoReadStatsSummary)); final nv = editingSettings.autoReadStatsSummary; parent.speak(parent._l10n.autoReadStatsSummaryState(nv ? parent._s('Zapnuto','On') : parent._s('Vypnuto','Off')), force: true); setState((){}); }, child: Text(parent._l10n.autoReadStatsSummaryState(s.autoReadStatsSummary ? parent._s('Zapnuto','On') : parent._s('Vypnuto','Off'))))),
            const Divider(),
            Semantics(label: parent._s('Přepnutí nápovědy pro pohyb ve statistickém souhrnu','Toggle stats summary navigation hint'), child: ElevatedButton(onPressed: () { _onUpdate((v)=> v.copyWith(showStatsNavigationHint: !v.showStatsNavigationHint)); final nv = editingSettings.showStatsNavigationHint; parent.speak(parent._l10n.statsNavigationHintState(nv ? parent._s('Zapnuto','On') : parent._s('Vypnuto','Off')), force: true); setState((){}); }, child: Text(parent._l10n.statsNavigationHintState(s.showStatsNavigationHint ? parent._s('Zapnuto','On') : parent._s('Vypnuto','Off'))))),
            const Divider(),
            Column(crossAxisAlignment: CrossAxisAlignment.start, children:[Semantics(header:true, child: Text(parent._s('Režim čtečky obrazovky','Screen reader mode'))), const SizedBox(height:8), SegmentedButton<ScreenReaderMode>(segments: [ButtonSegment(value: ScreenReaderMode.auto, label: Text(parent._s('Auto','Auto')), tooltip: parent._s('Automaticky podle čtečky','Automatic according to screen reader')), ButtonSegment(value: ScreenReaderMode.on, label: Text(parent._s('Zapnuto','On')), tooltip: parent._s('Režim čtečky obrazovky zapnut','Screen reader mode on')), ButtonSegment(value: ScreenReaderMode.off, label: Text(parent._s('Vypnuto','Off')), tooltip: parent._s('Režim čtečky obrazovky vypnut','Screen reader mode off'))], selected: {s.screenReaderMode}, onSelectionChanged: (Set<ScreenReaderMode> sel){ _onUpdate((v)=> v.copyWith(screenReaderMode: sel.first)); parent.speak(parent._s('Režim čtečky: ${sel.first.name}','Screen reader: ${sel.first.name}'), force: true); setState((){}); })]),
            const Divider(),
      ...
      actions: [
        ...
        Semantics(label: parent._s('Uložit změny profilu $editingName','Save changes for profile $editingName'), button:true, child: FilledButton(autofocus:true, onPressed: () async { await _confirmAndSave(); }, child: Text(parent._s('Uložit','Save')))),
      ],
      ),
    );
  }
